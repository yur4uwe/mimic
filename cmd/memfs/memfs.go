...
// helper to log callback entry with context
func logCb(name string, args ...interface{}) {
	uid, gid, _ := fuse.Getcontext()
	fmt.Printf("[CB] %s uid=%v gid=%v args=%#v\n", name, uid, gid, args)
}
...
func (self *Memfs) Getattr(path string, stat *fuse.Stat_t, fh uint64) (errc int) {
	logCb("Getattr (entry)", path, "fh", fh)
	defer trace(path, fh)(&errc, stat)
	defer self.synchronize()()
	node := self.getNode(path, fh)
	if nil == node {
		logCb("Getattr (result)", path, "ENOENT")
		return -fuse.ENOENT
	}
	*stat = node.stat
	logCb("Getattr (result)", path, "stat", *stat)
	return 0
}

func (self *Memfs) Truncate(path string, size int64, fh uint64) (errc int) {
	logCb("Truncate", path, size, "fh", fh)
...
func (self *Memfs) Read(path string, buff []byte, ofst int64, fh uint64) (n int) {
	logCb("Read", path, "buf_len", len(buff), "offset", ofst, "fh", fh)
	defer trace(path, buff, ofst, fh)(&n)
	defer self.synchronize()()
	node := self.getNode(path, fh)
	if nil == node {
		logCb("Read (result)", path, "ENOENT")
		return -fuse.ENOENT
	}
	...
	if endofst < ofst {
		logCb("Read (result)", path, "0 bytes (offset beyond size)")
		return 0
	}
	n = copy(buff, node.data[ofst:endofst])
	node.stat.Atim = fuse.Now()
	logCb("Read (result)", path, "read_bytes", n)
	return
}

func (self *Memfs) Write(path string, buff []byte, ofst int64, fh uint64) (n int) {
	logCb("Write", path, "buf_len", len(buff), "offset", ofst, "fh", fh)
	defer trace(path, buff, ofst, fh)(&n)
	defer self.synchronize()()
	node := self.getNode(path, fh)
	if nil == node {
		logCb("Write (result)", path, "ENOENT")
		return -fuse.ENOENT
	}
	endofst := ofst + int64(len(buff))
	if endofst > node.stat.Size {
	...
	node.stat.Ctim = tmsp
	node.stat.Mtim = tmsp
	logCb("Write (result)", path, "wrote_bytes", n, "new_size", node.stat.Size)
	return
}

func (self *Memfs) Release(path string, fh uint64) (errc int) {
	logCb("Release", path, "fh", fh)
...
func (self *Memfs) Readdir(path string,
	...
	ofst int64,
	fh uint64) (errc int) {
	logCb("Readdir", path, "offset", ofst, "fh", fh)
	...
	for name, chld := range node.chld {
		...
		logCb("Readdir (entry)", path, name, chld.stat)
	}
	logCb("Readdir (done)", path)
	return 0
}

func (self *Memfs) Releasedir(path string, fh uint64) (errc int) {
	logCb("Releasedir", path, "fh", fh)
	defer trace(path, fh)(&errc)
	defer self.synchronize()()
	return self.closeNode(fh)
}
...
func (self *Memfs) Setxattr(path string, name string, value []byte, flags int) (errc int) {
	logCb("Setxattr", path, name, "len", len(value), "flags", flags)
